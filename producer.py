...
if not os.path.exists(DATA_DIR):
    raise Exception(f"Директория {DATA_DIR} не существует")
...
csv_files = sorted([
    ...
    if f.lower().endswith('.csv') and f.startswith('mock_data')
])

if not csv_files:
    raise Exception(f"CSV файлы не найдены в директории {DATA_DIR}")
